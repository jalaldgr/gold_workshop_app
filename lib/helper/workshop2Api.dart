...
/////////////////////////// workshop1 ////////////////////////
  ...
  static Future<List<orderData>> getPendingOrdersByWorkshop2() async {
    ...
    final response = await http.get(
      Uri.parse('${dotenv.env['API_URL']}/workshop2/get-all-pending-orders/$userID/ارسال به کارگاه/'),
      headers: {'Authorization': 'Bearer $token'},
    );
  ...
  static Future<String> completeOrderWorkshop2(orderData order) async{
    ...
    final response = await http.post(
        Uri.parse('${dotenv.env['API_URL']}/workshop2/complete-order/${order.id}/برگشت از کارگاه'),
        headers: {'Authorization': 'Bearer $token'},
        body: order.toJson()
    );
    return response.body;
  ...
  static Future<List<orderData>> getOrders() async {
    ...
    dynamic token = prefs.getString("jwt");
    final response = await http.get(
      Uri.parse('${dotenv.env['API_URL']}/workshop2/get-all-order'),
      headers: {'Authorization': 'Bearer $token'},
    );
    List jsonResponse = json.decode(response.body);
    return jsonResponse.map((myMap) => orderData.fromJson(myMap)).toList();
  ...
  static Future<List<orderData>> getAllActiveOrders() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    dynamic token = prefs.getString("jwt");
    dynamic userID =jsonDecode(prefs.getString('user')!)['id'];
    final response = await http.get(
      Uri.parse('${dotenv.env['API_URL']}/workshop2/get-all-active-orders/'),
      headers: {'Authorization': 'Bearer $token'},
    );
    if (response.statusCode != 200) {
      List jsonResponse = json.decode(response.body) as List;
  ...
  static Future<String> returnOrderWorkshop2(orderData order) async{
    SharedPreferences prefs = await SharedPreferences.getInstance();
    dynamic token = prefs.getString("jwt");
